...
const Page = () => {
  ...
  const sortResumes = () => {
    const sortedResumes = [...resumes].sort((a, b) => b.percentage - a.percentage);
  ...
  return (
    <div className='p-8'>
      <div>
        ...
        <button className='border-2 border-black border-solid px-2 py-1 ml-2' onClick={uploadResume}>Upload Resume</button>
      ...
      {loading ? (
        ...
        <div>
          ...
          <button className='border-2 border-black border-solid px-2 py-1 mt-4' onClick={sortResumes}>Sort Resumes</button>
        </div>
      )}
    </div>
  )
